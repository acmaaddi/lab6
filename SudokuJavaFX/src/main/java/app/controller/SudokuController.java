...
public class SudokuController implements Initializable {
	...
	private Sudoku secondSudoku =null;
	private int[][] secondSudokuArray;
	...
	private void CreateSudokuInstance() {
		...
		secondSudoku = s; //tried to clone the sudoku to keep track of bad values behind the scenes
...
/*
		int[][] secondSudokuArray = new int[s.getiSize()][s.getiSize()]; //this hasnt helped yet
		
		for (int i = 0; i < s.getiSize(); i++) {
			for (int j = 0; j < s.getiSize(); j++) {
				secondSudokuArray[i][j] = s.getPuzzle()[i][j];
			}
		}
		*/
	...
	private GridPane BuildNumbersGrid() {
		...
		StackPane trashCanIcon= new StackPane();
		ImageView iv= new ImageView(this.GETtrashCan());
		iv.setFitHeight(50);
		//iv.setFitWidth(20);
		//it kept crashing our computers when we messed with the settings
		trashCanIcon.getChildren().add(iv);
		
		gridPaneNumbers.add(trashCanIcon,s.getiSize()+1,0);
...
/*
		trashCanIcon.setOnDragDropped(new EventHandler<DragEvent>() {
			public void handle(DragEvent event) {
				...
				//set the cell value to zero after its dropped 
				success = true;
				event.setDropCompleted(success);
				event.consume();
				
			// will also need to clear things
			}
		});
				*/
	...
	private GridPane BuildSudokuGrid() {
		...
		StackPane trashCanIcon= new StackPane(); //not sure how to make one single trashcan icon accessible between grids
		...
		for (int iCol = 0; iCol < s.getiSize(); iCol++) {
			...
			for (int iRow = 0; iRow < s.getiSize(); iRow++) {
				...
				if (s.getPuzzle()[iRow][iCol] != 0) {
					...
					paneTarget.getCell().setiCellValue(secondSudoku.getPuzzle()[iRow][iCol]);
				...
				paneTarget.setOnDragOver(new EventHandler<DragEvent>() {
					public void handle(DragEvent event) {
						if (event.getGestureSource() != paneTarget && event.getDragboard().hasContent(myFormat)) {
							...
							//lab6 edits go here
							if (paneTarget.getCell().getiCellValue() == 0) { //for some reason this zero value cant be changed without crashing ht egame
					...
					public void handle(DragEvent event) {
						...
						if (event.getGestureSource() != paneTarget && event.getDragboard().hasContent(myFormat)) {
							...
							if (CellTo.getiCellValue() == 0 ) {
								// we wanted to use this code in the above if statement && !s.isValidValue(CellTo.getiRow(), CellTo.getiCol())*/
					
								//we want it to be null but we cant figure out how to get it to refer to the correct map/ soduku array
				...
				//this code should make it such that something would be set to zero if dropped on trash can icon
	...
	private Image GETtrashCan() {
		InputStream can = getClass().getClassLoader().getResourceAsStream("img/trashCan.png");
